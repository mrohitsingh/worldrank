...
const MyCountries = ({countries}) => {
    ...
    const orderByPopulation = (countries) => {
        ...
        setAreaFlag(0);
        if (populationFlag == 0) {
            let pop = [...countries].sort((a,b) => (a.population < b.population ? 1 : -1));
            setPopulationFlag(1);
            setMyCountry(pop);
        }
        if (populationFlag == 1) {
    ...
    const orderByName = (countries) => {
        ...
        setAreaFlag(0);
        if (nameFlag == 0) {
            let nam = [...countries].sort((a,b) => (a.name.common < b.name.common ? 1 : -1));
            setNameFlag(1);
            setMyCountry(nam);
        }
        if (nameFlag == 1) {
    ...
    const handleInputChange = (e) => {
        ...
        setMyCountry(countries.filter(country => country.name.common.toLowerCase().includes(search)));   
    ...
    <div className={styles.countriestable_outer}>
        ...
        <div className={styles.countriestable_inner}>
            <div className={styles.head}>
                ...
                <button>Flags</button>
                <button onClick={() =>orderByArea(countries)}>Area
                    {
                        areaFlag == 0 ? <i className={`${styles.arrow} ${styles.down}`}></i> : <i className={`${styles.arrow} ${styles.up}`}></i>
                    }</button>
                <button onClick={() =>orderByPopulation(countries)}>Population
                    {
            ...
            <div>
                {
                    myCountry == 0 ? <div className={styles.no_result}>No result found</div> : 
                    <div className={styles.content}>
                        {
                    myCountry.map((country, index) => {
                        return (
                            <div key={index} className={styles.country_row} onClick={() => router.push({
                                pathname: `/countrypage/${country.ccn3}`,
                                
                            })}>
                                <div className={styles.country_name}><p>{country.name.common}</p></div>
                                <div className={styles.country_flag}><img src={country.flags.svg} alt={country.name.common} /></div>
                                <div className={styles.country_area}><p>{country.area}</p></div>
                                <div className={styles.country_population}><p>{country.population}</p></div>
                            </div>
                        )
                    }
                    )
